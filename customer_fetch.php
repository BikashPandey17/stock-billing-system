...
{
	$query .= 'WHERE customer.customer_name LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR customer.customer_status LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = '<button type="button" name="order" id="'.$row["customer_id"].'" class="btn btn-warning btn-xs order" data-status="'.$row["customer_status"].'">Create Order</button>';
	$data[] = $sub_array;
}
